...
def gausscheb(f, lam, lower, upper, n):
    # Inputs:
    ...
    for i in range(len(lam)):
        ...
        if (upper == np.inf):
            ...
            while abs(bdiff) > .01:
                b_1 = b_0 * 2
                result_0 = Gc(f, lower, b_0, x, w, ptup)
                result_1 = Gc(f, lower, b_1, x, w, ptup)
                bdiff = result_1 - result_0
                b_0 = b_1
        else:
            b_1 = up

        # Call Gc function from above to find the value of the integral.
        gausschebsol = Gc(f, lower, b_1, x, w, ptup)

        # Store results
        solution[i] = gausschebsol
